...
export default function AnggaranPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const payload = { item_name: itemName, planned_amount: Number(plannedAmount) };
  ...
  const handleDelete = async (id) => {
    if (confirm('Hapus mata alokasi anggaran ini?')) {
        ...
        {budgets.map(b => (
          <div key={b.id} className="p-3 bg-slate-950 border border-slate-800 rounded-xl flex justify-between items-center text-xs">
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => handleEdit(b)} className="text-amber-500 hover:underline">Edit</button>
              <button onClick={() => handleDelete(b.id)} className="text-rose-400 hover:underline">Hapus</button>
            </div>
          </div>
        ))}
      </div>
