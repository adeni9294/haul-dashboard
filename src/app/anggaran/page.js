    ...
    try {
      ...
      if (tData) {
        tData.forEach(item => {
          ...
          const isKeluar = rawType === 'keluar' || rawType === 'pengeluaran';
          if (isKeluar) {
            const cat = (item.category || item.kategori || 'Lain-lain').toString().trim().toLowerCase();
            const nominal = Math.abs(parseFloat(item.amount || item.nominal) || 0);
            expMap[cat] = (expMap[cat] || 0) + nominal;
          }
        });
      }

      setExpenseSummary(expMap);
      setBudgetList(bData || []);
    } catch (e) {
      console.error(e);
    } finally {
  ...
  return (
    <div className="space-y-4 max-w-7xl mx-auto px-1 sm:px-0 pb-12 text-xs text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* LIST DAFTAR RENCANA ANGGARAN, REALISASI, DAN SELISIH */}
                ...
                return (
                  <div key={b.id} className="p-3.5 bg-black/20 border border-white/10 rounded-xl space-y-2 hover:border-white/30 transition-all">
                    ...
                    <div className="grid grid-cols-3 gap-2 pt-1 font-mono text-[10px] border-t border-white/10">
                      ...
                      <div>
                        <span className="opacity-60 block text-[9px] uppercase">Realisasi:</span>
