...
export default function KepanitiaanPage() {
  ...
  const handleDelete = async (id) => {
    if (confirm('Hapus nama pengurus ini dari daftar?')) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <form onSubmit={handleSubmit} className="p-6 bg-slate-900 border border-slate-800 rounded-2xl h-fit space-y-4">
        <h3 className="text-xs font-bold text-amber-500 uppercase">{editingId ? '🔄 Koreksi Struktur' : '👥 Tambah Anggota Panitia'}</h3>
        ...
        {members.map(m => (
          <div key={m.id} className="p-3 bg-slate-950 border border-slate-800 rounded-xl flex justify-between items-center text-xs">
            <div>
              ...
              {m.phone_number && <p className="text-[10px] text-slate-500">📞 {m.phone_number}</p>}
            </div>
            <div className="flex gap-3">
              <button onClick={() => handleEdit(m)} className="text-amber-500 hover:underline">Edit</button>
              <button onClick={() => handleDelete(m.id)} className="text-rose-400 hover:underline">Hapus</button>
            </div>
          </div>
        ))}
      </div>
