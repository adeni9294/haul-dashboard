...
export default function RootLayout({ children }) {
  ...
  return (
    <html lang="id">
      <body className={`${currentStyle.body} font-sans min-h-screen flex flex-col transition-all duration-300`}>
        ...
        <div className="w-full max-w-7xl mx-auto px-4 pt-4">
          ...
          <nav className={`grid grid-cols-3 sm:flex sm:flex-wrap items-center gap-1.5 ${currentStyle.navBg} border p-1.5 rounded-2xl text-[11px] sm:text-xs font-bold w-full sm:w-fit backdrop-blur-sm text-center`}>
            ...
            <Link href="/anggaran" className={`py-2 px-1 sm:px-4 rounded-xl transition-all truncate ${pathname === '/anggaran' ? 'bg-amber-500 text-slate-950 shadow-md font-black' : `${currentStyle.textMuted} hover:text-white`}`}>📈 Anggaran</Link>
            <Link href="/acara" className={`py-2 px-1 sm:px-4 rounded-xl transition-all truncate ${pathname === '/acara' ? 'bg-amber-500 text-slate-950 shadow-md font-black' : `${currentStyle.textMuted} hover:text-white`}`}>📅 Acara</Link>
            <Link href="/kepanitiaan" className={`py-2 px-1 sm:px-4 rounded-xl transition-all truncate ${pathname === '/kepanitiaan' ? 'bg-amber-500 text-slate-950 shadow-md font-black' : `${currentStyle.textMuted} hover:text-white`}`}>👥 Panitia</Link>
            {isAdmin && (
              <Link href="/pengaturan" className={`py-2 px-1 sm:px-4 rounded-xl transition-all truncate ${pathname === '/pengaturan' ? 'bg-amber-500 text-slate-950 shadow-md font-black' : `${currentStyle.textMuted} hover:text-white`}`}>⚙️ Pengaturan</Link>
            )}
          </nav>
        </div>
