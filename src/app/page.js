    ...
    <div className="space-y-4.5 max-w-7xl mx-auto px-1 sm:px-0 pb-16">
      ...
      <div className={`p-4 sm:p-5 ${style.card} border rounded-xl sm:rounded-2xl space-y-2.5`}>
        <div className="flex justify-between items-center">
          ...
          <span className="px-1.5 py-0.5 bg-amber-500/20 text-amber-400 font-mono text-[10px] font-black rounded border border-amber-500/40">
            {progress.percent}%
          </span>
        </div>
              ...
              {rincianMasuk.length > 0 ? rincianMasuk.map((item, idx) => (
                <div key={item.id || idx} className="flex justify-between items-center pt-2 first:pt-0 gap-2">
                  <div className="min-w-0 flex-1 space-y-0.5">
                    <p className="text-slate-100 font-bold text-xs truncate">{item.description || item.keterangan || item.notes || 'Transaksi Masuk'}</p>
                    <p className={`text-[9px] font-mono ${style.textMuted} font-medium`}>{item.transaction_date || item.tanggal || '-'}</p>
                  </div>
                  <p className="text-xs font-black font-mono text-emerald-400 shrink-0">+{formatRupiah(item.amount || item.nominal)}</p>
                </div>
              )) : (
          ...
          <div className={`p-4 ${style.card} border-l-4 border-l-rose-500 border-y border-r rounded-xl space-y-2.5`}>
            ...
            <div className="space-y-2 divide-y divide-slate-800 pr-0.5">
              {rincianKeluar.length > 0 ? rincianKeluar.map((item, idx) => (
                <div key={item.id || idx} className="flex justify-between items-center pt-2 first:pt-0 gap-2">
                  <div className="min-w-0 flex-1 space-y-0.5">
                    <p className="text-slate-100 font-bold text-xs truncate">{item.description || item.keterangan || item.notes || 'Transaksi Keluar'}</p>
                    <p className={`text-[9px] font-mono ${style.textMuted} font-medium`}>{item.transaction_date || item.tanggal || '-'}</p>
                  </div>
                  <p className="text-xs font-black font-mono text-rose-400 shrink-0">-{formatRupiah(item.amount || item.nominal)}</p>
                </div>
              )) : (
