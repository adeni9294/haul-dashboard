...
export default function DashboardPage() {
    ...
    async function loadDashboardData() {
      ...
      try {
        ...
        if (configData && configData.length > 0) {
          if (configData[0].announcement) setAnnouncement(configData[0].announcement);
        }
  ...
  if (loading) return <div className="text-center py-20 text-xs font-mono text-slate-400 animate-pulse">⏳ Sinkronisasi papan kendali...</div>;

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-bold text-white">📊 Dashboard Ringkasan Panitia</h2>
        <p className="text-xs text-slate-400">Pantau pergerakan kas live, target capaian, dan log sirkulasi keuangan.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 rounded-2xl">
          <p className="text-[10px] uppercase font-bold text-amber-500 tracking-wider mb-1">💰 Sisa Saldo Kas</p>
        ...
        <div className="p-5 bg-slate-900/60 border border-slate-800 rounded-2xl space-y-2">
          <div className="flex justify-between items-center">
            <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">🎯 Target Anggaran</p>
              ...
              {recentTransactions.map((t, idx) => {
                ...
                return (
                  <tr key={idx} className="hover:bg-slate-950/40">
                    ...
                    <td className="py-2.5">
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase ${isPemasukan ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>{isPemasukan ? 'Masuk' : 'Keluar'}</span>
                    </td>
                    <td className={`py-2.5 text-right font-mono font-bold ${isPemasukan ? 'text-emerald-400' : 'text-rose-400'}`}>{isPemasukan ? '+' : '-'}{formatRupiah(t.amount)}</td>
                  </tr>
                );
              })}
            </tbody>
