...
export default function PengaturanPage() {
  ...
  if (!isAdmin) {
    return (
      <div className="p-8 text-center theme-bg-secondary border theme-border rounded-3xl max-w-md mx-auto my-12 shadow-2xl space-y-3">
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
      ...
      {/* HEADER PAGE */}
      <div className="theme-bg-secondary border theme-border p-5 rounded-3xl shadow-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 transition-all duration-300">
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          ...
          {/* SEKSI 1: TEMA GLOBAL */}
          <div className="theme-bg-secondary border theme-border p-5 sm:p-6 rounded-3xl shadow-xl space-y-4">
                ...
                return (
                  <button
                    ...
                    className={`p-2.5 rounded-2xl border text-left transition-all duration-200 flex items-center justify-between gap-1.5 ${
                      isSelected 
                        ? 'theme-bg-tertiary border-cyan-400 shadow-md scale-[1.02]' 
                        : 'theme-bg-tertiary/40 theme-border opacity-70 hover:opacity-100'
                    }`}
                  >
          ...
          <form onSubmit={handleSaveConfig} className="theme-bg-secondary border theme-border p-5 sm:p-6 rounded-3xl shadow-xl space-y-4">
            <h3 className="text-xs font-extrabold uppercase tracking-wider theme-text-primary border-b theme-border pb-3 flex items-center gap-2">
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <input 
                  ...
                  className="w-full px-4 py-2.5 theme-bg-tertiary border theme-border theme-text-primary rounded-2xl focus:outline-none focus:border-cyan-500 font-semibold" 
                />
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full px-4 py-2.5 theme-bg-tertiary border theme-border theme-text-primary rounded-2xl focus:outline-none focus:border-cyan-500" 
                />
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3 p-3 theme-bg-tertiary border theme-border rounded-2xl">
                  <div className="w-12 h-12 rounded-2xl border theme-border theme-bg-secondary overflow-hidden shrink-0 flex items-center justify-center">
                    {logoUrl ? <img src={logoUrl} alt="Logo" className="w-full h-full object-cover" /> : <span className="text-[9px] theme-text-secondary">NO LOGO</span>}
                  </div>
                  <div className="flex-1 min-w-0">
                    <input type="file" accept="image/*" id="upload-logo-input" onChange={handleUploadLogo} disabled={isUploading} className="hidden" />
                    <label htmlFor="upload-logo-input" className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] font-extrabold uppercase tracking-wider cursor-pointer transition-all ${isUploading ? 'bg-white/10 text-slate-400' : 'theme-gradient-main theme-text-primary hover:opacity-90'}`}>
                      <Upload className="w-3.5 h-3.5" />
                      {isUploading ? 'Mengunggah...' : 'Pilih Logo'}
                    </label>
                    <p className="text-[9px] theme-text-secondary mt-1 truncate">{logoUrl || 'Belum ada logo diunggah'}</p>
                  </div>
                </div>
              </div>
            </div>

            <button type="submit" className="w-full py-3 theme-gradient-main theme-text-primary font-black text-xs uppercase tracking-wider rounded-2xl shadow-lg transition-all hover:scale-[1.01] active:scale-95 flex items-center justify-center gap-2">
          ...
          <div className="theme-bg-secondary border theme-border p-5 sm:p-6 rounded-3xl shadow-xl space-y-4">
            ...
            <form onSubmit={handleSavePeriode} className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                {editingPeriodeId && (
                  <button type="button" onClick={() => { setEditingPeriodeId(null); setNamaPeriodeInput(''); setSaldoAwalInput(''); }} className="px-4 py-2.5 theme-bg-tertiary border theme-border theme-text-secondary rounded-2xl text-xs">Batal</button>
                )}
              </div>
            </form>
        ...
        <div className="space-y-6">
          ...
          <form onSubmit={handleUpdatePassword} className="theme-bg-secondary border theme-border p-5 sm:p-6 rounded-3xl shadow-xl space-y-4">
            <h3 className="text-xs font-extrabold uppercase tracking-wider theme-text-primary border-b theme-border pb-3 flex items-center gap-2">
      ...
      {/* 🔔 CUSTOM MODAL TOAST DIALOG (SAMA PRESISI DENGAN CLIENTLAYOUT) */}
      {toastConfig.show && (
          ...
          <div className="theme-bg-secondary border theme-border p-6 rounded-3xl w-full max-w-sm space-y-4 shadow-2xl theme-text-primary text-center relative overflow-hidden transition-all duration-300">
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className="flex-1 py-3 theme-bg-tertiary border theme-border theme-text-secondary font-bold text-xs uppercase rounded-2xl"
              >
                Batal
              </button>
              <button
